...
print("[Gradio App DEBUG] Script started, attempting to load environment variables and setup logging.")

# Check if ffmpeg is available
try:
    ffmpeg_path = shutil.which("ffmpeg")
    if ffmpeg_path:
        print(f"[Gradio App DEBUG] FFmpeg found at: {ffmpeg_path}")
        result = subprocess.run([ffmpeg_path, "-version"], capture_output=True, text=True, check=True)
        print(f"[Gradio App DEBUG] FFmpeg version output: {result.stdout.splitlines()[0]}")
    else:
        print("[Gradio App DEBUG] FFmpeg not found in PATH. Please ensure it is installed and in PATH.")
except FileNotFoundError:
    print("[Gradio App DEBUG] FFmpeg executable not found. Please ensure it is installed and in PATH.")
except subprocess.CalledProcessError as e:
    print(f"[Gradio App DEBUG] Error running FFmpeg: {e}")
except Exception as e:
    print(f"[Gradio App DEBUG] Unexpected error checking FFmpeg: {e}")
...
print(f"[Gradio App DEBUG] Orchestrator URL being used: {ORCHESTRATOR_URL}")
...
def process_audio_and_generate_brief(audio):
    ...
    print(f"[Gradio App DEBUG] Type of audio input: {type(audio)}")
    ...
    try:
        ...
        if audio is None:
            ...
            return "Please record or upload audio first.", None, None

        # Gradio gives audio as (sample_rate, np_array)
        sample_rate, audio_data = audio
        print(f"[Gradio App DEBUG] Sample rate: {sample_rate}, Audio data shape: {audio_data.shape}")
        ...
        try:
            ...
            print("[Gradio App ERROR] ConnectionError: Could not connect to the Orchestrator.")
            return "Could not connect to the Orchestrator. Please ensure it is running.", None, None
        except Exception as e:
            print(f"[Gradio App ERROR] An unexpected error occurred in process_audio_and_generate_brief: {e}")
            import traceback
...
with gr.Blocks() as demo:
    ...
    audio_input = gr.Audio(label="Speak your query", sources=["microphone", "upload"], streaming=False)
    ...
    print("[Gradio App DEBUG] Audio input event listeners set up.")
...
if __name__ == "__main__":
    print(f"[Gradio App DEBUG] Final Orchestrator URL: {ORCHESTRATOR_URL}")
